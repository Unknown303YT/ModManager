package com.riverstone.unknown303.modmanager.common.networking;
...
public class FriendlyByteBuf extends ByteBuf {
     ...
     * @see #writeCollection
     */
    public <T, C extends Collection<T>> C readCollection(IntFunction<C> pCollectionFactory, Reader<T> pElementReader) {
     ...
     * @see #readCollection
     */
    public <T> void writeCollection(Collection<T> pCollection, Writer<T> pElementWriter) {
     ...
     * @see #writeCollection
     */
    public <T> List<T> readList(Reader<T> pElementReader) {
     ...
     * @see #writeMap
     */
    public <K, V, M extends Map<K, V>> M readMap(IntFunction<M> pMapFactory, Reader<K> pKeyReader, Reader<V> pValueReader) {
        int i = this.readVarInt();
     ...
     * @see #writeMap
     */
    public <K, V> Map<K, V> readMap(Reader<K> pKeyReader, Reader<V> pValueReader) {
     ...
     * @see #readMap
     */
    public <K, V> void writeMap(Map<K, V> pMap, Writer<K> pKeyWriter, Writer<V> pValueWriter) {
    ...
    public <T> void writeOptional(Optional<T> pOptional, Writer<T> pWriter) {
    ...
    public <T> Optional<T> readOptional(Reader<T> pReader) {
        return this.readBoolean() ? Optional.of(pReader.apply(this)) : Optional.empty();
    }

    @Nullable
    public <T> T readNullable(Reader<T> pReader) {
        return (T)(this.readBoolean() ? pReader.apply(this) : null);
    }

    public <T> void writeNullable(@Nullable T pValue, Writer<T> pWriter) {
        if (pValue != null) {
            this.writeBoolean(true);
     ...
     * @see #readIdentifier
     */
    public FriendlyByteBuf writeIdentifier(Identifier pIdentifier) {
        this.writeUtf(pIdentifier.toString());
        return this;
    }
    ...
    public BitSet readFixedBitSet(int pSize) {
        byte[] abyte = new byte[-Math.floorDiv(-pSize, 8)];
        this.readBytes(abyte);
    ...
    public void writeFixedBitSet(BitSet pBitSet, int pSize) {
        if (pBitSet.length() > pSize) {
            ...
            this.writeBytes(Arrays.copyOf(abyte, -Math.floorDiv(-pSize, 8)));
        }
    }
